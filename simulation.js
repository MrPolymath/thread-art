...
function drawLines(vertices){
  ...
  for (var i = 0; i < n; i++) {
    ...
    for (var j = 0; j < n; j++) {
      ...
      ctx.stroke()
      dist+= Math.sqrt((centerX+vertices[j][0]-startX)*(centerX+vertices[j][0]-startX)+(centerY-vertices[j][1]-centerY+vertices[i][1])*(centerY-vertices[j][1]-centerY+vertices[i][1]))
    }
  }
console.log(dist);
}
